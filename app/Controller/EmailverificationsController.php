...
class EmailverificationsController extends AppController
{
    ...
    public function EmailVerify($id)
    {
        ...
        $post = $this->Member->findByRegCodeAndRegStatus($id,"Live");
        if($post)
        {
            ...
            if($guest_login==1)
            $this->request->data['Member']['reg_status']="Pending";
            else
            $this->request->data['Member']['status']="Active";
            $this->Member->id = $post['Member']['id'];
    ...
    public function resendsub()
    {
        ...
        $post = $this->Member->findByEmail($id);
        if($post)
        {
            if($post['Member']['reg_status']=="Live")
            {
                $memberName=$post['Member']['name'];
                $code=$post['Member']['reg_code'];
