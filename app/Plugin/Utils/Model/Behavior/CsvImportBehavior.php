		...
		while (($row = $this->_getCSVLine($Model, $handle)) !== false) {
			...
			if($type!=null)
			{
				...
				elseif($type=="StudentGroup")
				{
					...
					{
						$data1[]=array($type=>array($fixedId=>$Model->id,'group_id'=>$groupId));
						$email=$data['Iestudent']['email'];$studentName=$data['Iestudent']['name'];$mobileNo=$data['Iestudent']['phone'];$password="demo@123";
						if($siteArr['emailNotification']==1)
						{
							$email=$data['Iestudent']['email'];$studentName=$data['Iestudent']['name'];
							$password="demo@123";
							$emailMessage="Dear ".$studentName.",<br><br>Congratulations! Your ".$siteArr['siteName']." account is now active.
							...
							To get started, log on:<a target=\"_blank\" href=\"".$siteArr['siteDomain']."\">".$siteArr['siteDomain']."</a><br><br>
							If you have any questions or need assistance, please contact us.<br><br>Best Regards,<br>".$siteArr['siteName']." Support Team<br>
							".$siteArr['siteEmail']."<br>".$siteArr['siteEmailContact'];
							if($email)
							{
								$toEmail=array($email=>$studentName);
								$Email = new CakeEmail();
								$Email->from(array($siteArr['siteEmail'] =>$siteArr['siteName']));
								$Email->to($toEmail);
								$Email->emailFormat('html');
								$Email->subject($siteArr['siteName'].' candidate login credentials');
								$Email->send($emailMessage);
							}
						}
						if($siteArr['smsNotification']==1)
						{
							$smsMessage="Hi, ".$studentName.", Email: ".$email." Password: ".$password." Url:".$siteArr['siteDomain'];
							$Sms = new AppModel();
