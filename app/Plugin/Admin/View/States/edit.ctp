<div <?php if(!$isError){?>class="container"<?php }?>>    
    <div class="panel panel-custom mrg">
        <div class="panel-heading"><?php echo __('Edit State');?><?php if(!$isError){?><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><?php }?></div>
        <div class="panel-body"><?php echo $this->Session->flash();?>
	    <?php echo $this->Form->create('State',array('class'=>'form-horizontal'));?>
	    <?php foreach ($State as $k=>$post): $id=$post['State']['id'];$form_no=$k+1;?>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
			  <?php  echo $this->Form->select("$k.State.country_id",$country,array('required'=>true,'class'=>'form-control','empty'=>'Select'));?>
			</div>
		    </div>
		    <div class="form-group">
			<label for="group_name" class="col-sm-3 control-label"><?php echo __('Name');?></label>
			<div class="col-sm-9">
			   <?php echo $this->Form->input("$k.State.name",array('label' => false,'class'=>'form-control','placeholder'=> __('Name'),'div'=>false));?>
			</div>
		    </div>
		    <div class="form-group text-left">
			<div class="col-sm-offset-3 col-sm-6">
			    <?php echo $this->Form->input("$k.State.id", array('type' => 'hidden'));?>
			</div>
		    </div>
		</div>
	    </div>
