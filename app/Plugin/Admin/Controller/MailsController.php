...
class MailsController extends AdminAppController {
    ...
    public function compose($type=null,$id=null)
    {
        ...
        $this->set('memberId',$this->Member->find('list',array('fields'=>array('user_name','id','name','email'))));        
        if ($this->request->is('post'))
        {
            try
            {
               $post=$this->request->data['Mail']['to_email'];
                ...
                {
                    $this->Session->setFlash(__('Member Email not found'),'flash',array('alert'=>'danger'));
                }
            }
            catch (Exception $e)
            {
        ...
        {
             $this->loadModel('Member');
             $postMember = $this->Member->findById($id);
             if($postMember)
             {
                $this->request->data['Mail']['to_email']=$postMember['Member']['user_name'];
             }   
        }
        else
        {
    ...
    public function membersearch()
    {
        ...
        $users = $this->Member->find('all',array('conditions' => array('Member.user_name LIKE' => '%'.$term.'%')));
        // Format the result for select2
        $result = array();
        foreach($users as $key => $user)
        {
            $result[$key]['id'] = $user['Member']['user_name'];
            $result[$key]['text'] = $user['Member']['user_name'];
        }
        $users = $result;        
